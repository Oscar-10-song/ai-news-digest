  ...
  <div style="background:#fff;border-radius:12px;padding:30px;box-shadow:0 2px 8px rgba(0,0,0,.06)">
    ...
    <div style="margin-top:30px;padding-top:16px;border-top:1px solid #eee;text-align:center">
      <p style="color:#999;font-size:11px;margin:0">
        本邮件由 <a href="https://github.com" style="color:#2563eb">AI-News-Digest</a> 自动生成 ·
        每天 8:00 (北京时间) 发送 ·
        <a href="#" style="color:#999">退订</a>
      </p>
    </div>
  </div>
</body></html>`;
...
export async function sendEmail(
  ...
  try {
    console.error("  [resend] 发送邮件到", toEmail, "...");

    const resend = new Resend(apiKey);

    const { data, error } = await resend.emails.send({
      from: "AI 前沿日报 <onboarding@resend.dev>",
      to: [toEmail],
    ...
    console.error("  [resend] 发送异常:", error);
    return false;
  }
}
